...
from analysis.src.featureFunctions import *
...
def getTrainData(ticker,start_date,end_date,options,assignLabels=labels3):
	...
	:raises: None
	"""
    #print('in getTrainData')
    start_date=datetime.strptime(start_date,"%Y-%m-%d").date()
    end_date=datetime.strptime(end_date,"%Y-%m-%d").date()
    #print('getting raw data for training')
    tickerDataRaw=getRawData(ticker,start_date,end_date,options)
    #print('raw data for training returned')
    tickerDataRaw["Return"]=getReturn(tickerDataRaw["Price"])
    #print('returns retrievd')
    #print (tickerDataRaw["Return"].head(5))
