"""
This module will contain all the functions required to construct the features that we want to use, using the rawdata
...
def getHistory(returnSeries,start_date,history=3):
	...
	"""
    #print('in getHistory')
    # This will add the last 3 periods returns as features, given the return series of the datapoints in question
    #print('returnSeries.shape: ', returnSeries.head())
    dates = np.array(returnSeries.index)
    #print ('datesShape: ' , dates[0], dates[1], dates[2], dates[3],dates[4])
    start_idx = np.where(dates>=start_date)[0][-1]
    #print ('startIdx: ', dates[start_idx])
    ...
    #print('dateIndex: ' , dateIndex[start_idx])
    # This creates an index which only contains the dates we care about, the trailing dates are removed
    ...
    for i in range(history):
        ...
        returnHistory=pd.Series(returnSeries[i+1:start_idx+2+i])
        #print('returnHistory.shape: ' , returnHistory.shape)
        #print ('dateIndex.shape: ' , dateIndex.shape)
        ...
        #print ('historyDict.shape: ' , historydict)
...
def getMomentum(tickerDataRawDaily,period=25):
    ...
    logReturns["Mom_1"]=logReturns["logRtn"].rolling(period,min_periods=2).apply(np.nansum)
    logReturns["Mom_2"]=logReturns["Mom_1"]/logReturns["logRtn"].rolling(period,min_periods=2).apply(np.nanstd)
...
def getJump(tickerDataRaw,period=[3,25]):
    ...
    logReturns["Jmp_1"]=logReturns["logRtn"].rolling(period[0],min_periods=2).apply(np.nanmean)-\
                        logReturns["logRtn"].rolling(period[1],min_periods=2).apply(np.nanmean)
    logReturns["Jmp_2"]=logReturns["Jmp_1"]/logReturns["logRtn"].rolling(period[1],min_periods=2).apply(np.nanstd)
...
def getPrevWeeks(tickerDataRaw):
    ...
    #print(logReturns.head())
    logReturns["prevWeeks"] = logReturns["logRtn"].rolling(window=period, min_periods=2,center=True).apply(daysToWeeks)
   ...
   # The function daysToWeeks is where the trends in the past 4 weeks are identified and coded into a 4 digit integer
...
def daysToWeeks(dailyRtns):
	...
	:raises: None
	"""
    #print('in daystoweeks')
    #print(dailyRtns.size)
    weekSignals="0"
    for i in range(int(dailyRtns.size/6)):
